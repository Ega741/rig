...
async function runSearch(engine: Engine, seconds: number, intensity = 100): Promise<SearchReport> {
  const v = vectors[0]!;
  const beneficiary = getAddress(v.beneficiary) as Address;
  const challenge = v.challenge as Hex;
  const hits: Hit[] = [];
  engine.onHit = (hit) => hits.push(hit);
  engine.start({ header: headerBytes(beneficiary, challenge), segment: 42n, difficulty: SEARCH_DIFFICULTY, intensity });
  await new Promise((resolve) => setTimeout(resolve, seconds * 1000));
  const rate = engine.hashRate();
  const duty = engine.dutyCycle();
  engine.stop();
  ...
  return { hits: hits.length, verified, invalid, rate, duty, seconds };
}

async function gpu(intensity = 100): Promise<{ available: boolean; adapter?: unknown; vectorsOk?: number; vectorsTotal?: number; search?: SearchReport }> {
  ...
  const search = await runSearch(engine, 3, intensity);
  return { available: true, adapter: engine.adapterInfo, vectorsOk, vectorsTotal: vectors.length, search };
}

async function cpu(cores: number, intensity = 100): Promise<{ search: SearchReport; errors: string[] }> {
  const engine = new CpuEngine(cores);
  const errors: string[] = [];
  engine.onError = (worker, message) => errors.push(`worker ${worker}: ${message}`);
  const search = await runSearch(engine, 4, intensity);
  return { search, errors };
}

declare global {
  interface Window {
    rigTest: { gpu: typeof gpu; cpu: typeof cpu };
  }
}
window.rigTest = { gpu, cpu };
